...
namespace IO::ADT
{
                                                    ...
                                                    , std::size_t size)
  {
    ...
    RequireF(CCodeZones::FILE_IO
             , read_ctx.alpha_layer_params.Size() > 0
               && read_ctx.alpha_layer_params.Size() <= Common::WorldConstants::CHUNK_MAX_TEXTURE_LAYERS
    ...
    else
    {
      for (std::size_t layer_idx = 1; layer_idx < read_ctx.alpha_layer_params.Size(); ++layer_idx)
      {
        ...
        for (std::size_t i = 0; i < Common::WorldConstants::ALPHAMAP_DIM; ++i)
        {
          for (std::size_t j = 0; j < Common::WorldConstants::ALPHAMAP_DIM; j += 2)
          {
            alphamap[i * Common::WorldConstants::ALPHAMAP_DIM + j] =
              ((*raw_buffer & 0x0f) << 4) | (*raw_buffer & 0x0f);

            alphamap[i * Common::WorldConstants::ALPHAMAP_DIM + j + 1] =
              ((*raw_buffer & 0xf0) >> 4) | (*raw_buffer & 0xf0);
            raw_buffer++;
          }
        }
        ...
        {
          constexpr std::uint32_t last_pixel_idx = Common::WorldConstants::ALPHAMAP_DIM - 1;
          constexpr std::uint32_t pre_last_pixel_idx = last_pixel_idx - 1;

          for (std::size_t i = 0; i < Common::WorldConstants::ALPHAMAP_DIM; ++i)
          {
            alphamap[i * Common::WorldConstants::ALPHAMAP_DIM + last_pixel_idx]
              = alphamap[i * Common::WorldConstants::ALPHAMAP_DIM + pre_last_pixel_idx];
            alphamap[last_pixel_idx * Common::WorldConstants::ALPHAMAP_DIM + i]
              = alphamap[pre_last_pixel_idx * Common::WorldConstants::ALPHAMAP_DIM + i];
          }
          // handle corner pixel
          alphamap[pre_last_pixel_idx * Common::WorldConstants::ALPHAMAP_DIM + last_pixel_idx]
            = alphamap[pre_last_pixel_idx * Common::WorldConstants::ALPHAMAP_DIM + pre_last_pixel_idx];
        }
      }

      // normalize alpha for highres format
      for (std::size_t i = 0; i < Common::WorldConstants::N_PIXELS_PER_ALPHAMAP; ++i)
      {
  ...
  {
    RequireF(CCodeZones::FILE_IO
             , !_data.empty() && _data.size() < Common::WorldConstants::CHUNK_MAX_TEXTURE_LAYERS
             , "Only 3 alpha layers is supported.");

    RequireF(CCodeZones::FILE_IO, _data.size() == (write_ctx.alpha_layer_params.Size() - 1)
             , "Layers params size mismatch.");

    Common::ChunkHeader header {Common::FourCC<"MCAL">, 0};
                                                      ...
                                                      , write_ctx.alpha_layer_params.end())))
      {
        ...
        else
        {
          for (std::size_t i = 0; i < Common::WorldConstants::ALPHAMAP_DIM; ++i)
          {
            ...
            auto& cur_block = compression_blocks.emplace_back(
              std::pair{alphamap[i * Common::WorldConstants::ALPHAMAP_DIM], 1});

            for (std::size_t j = 1; j < Common::WorldConstants::ALPHAMAP_DIM; ++j)
            {
              std::uint8_t cur_pixel = alphamap[i * Common::WorldConstants::ALPHAMAP_DIM + j];
    ...
    else
    {
      ...
      std::vector<std::array<std::uint8_t, Common::WorldConstants::N_PIXELS_PER_ALPHAMAP>> temp_layers{};
      ...
      {
        for (std::size_t i = 0; i < Common::WorldConstants::N_BYTES_PER_LOWRES_ALPHA; ++i)
        {
